...
export function Combobox({
  ...
  const [open, setOpenBox] = React.useState(false);
  const [query, setQueryBox] = React.useState<string>("");

  return (
    <div className={cn('block', className)}>
      <Popover open={open} onOpenChange={setOpenBox}>
        ...
        <PopoverContent className='w-72 max-w-sm p-0'>
          ...
          >
            <CommandInput
              ...
              value={query}
              onValueChange={(value: string) => setQueryBox(value || "")}
            />
            <CommandEmpty
              onClick={() => {
                if (onCreate) {
